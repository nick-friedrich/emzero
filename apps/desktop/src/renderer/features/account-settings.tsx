...
import type { AccountSummary, AccountUpdate } from '../../shared/accounts';
import { AccountColorPicker } from './account-colors';
...
function AccountsSettings({ accounts, onChange }: { accounts: AccountSummary[]; onChange: (accounts: AccountSummary[]) => void }) {
  ...
  const rename = async (account: AccountSummary) => {
    ...
    await update(account, { name });
  };
  const update = async (account: AccountSummary, values: AccountUpdate) => {
    setBusy(account.id); setStatus(null);
    try { const result = await window.emzero.accounts.update(account.id, values); if (result.ok && result.account) { onChange(accounts.map((item) => item.id === account.id ? result.account! : item)); setStatus({ kind: 'success', message: result.message }); notifyAccountsChanged(); } else setStatus({ kind: 'error', message: result.message }); }
  ...
  return <><Heading title="Accounts" description="Rename, color, or remove connected mail accounts. The color marks each account's mail in the unified inbox." /><div className="space-y-3">{accounts.map((account) => <div key={account.id} className="rounded-lg border border-border bg-card p-4"><div className="mb-3"><p className="text-sm font-medium">{account.name}</p><p className="text-xs text-muted-foreground">{account.email}</p></div><div className="mb-4"><AccountColorPicker label={`Color for ${account.email}`} value={account.color} disabled={busy === account.id} onChange={(color) => { if (color !== account.color) void update(account, { color }); }} /></div><form className="flex gap-2" onSubmit={(event) => { event.preventDefault(); void rename(account); }}><input className="field min-w-0 flex-1" aria-label={`Name for ${account.email}`} value={names[account.id] ?? account.name} disabled={busy === account.id} onChange={(event) => setNames((current) => ({ ...current, [account.id]: event.target.value }))} /><Button type="submit" variant="secondary" disabled={busy === account.id || !names[account.id]?.trim() || names[account.id]?.trim() === account.name}>Save</Button><AlertDialog><AlertDialogTrigger asChild><Button type="button" variant="ghost" className="px-3 text-danger hover:text-danger" aria-label={`Delete ${account.email}`}><Trash2 className="size-4" /></Button></AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Delete {account.name}?</AlertDialogTitle><AlertDialogDescription>This removes the account and its locally cached mail. It does not delete mail from the provider.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => void remove(account)}>Delete account</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></form></div>)}</div><StatusMessage status={status} /></>;
}
